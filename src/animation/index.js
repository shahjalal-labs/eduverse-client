import authLottie from "./auth-lottie.json";
import createLottie from "./create.json";
import evaluateLottie from "./evaluate.json";
import submitLottie from "./submit.json";
import updateLottie from "./update-lottie.json";
import updateManLottie from "./update-man-lottie.json";
import gradient from "./gradient.json";
import circle from "./circle-zoom-in-out.json";
import coneLottie from "./cone-lottie.json";
import handLottie from "./hand.json";
import createLottie2 from "./createLottie2.json";

export {
  authLottie,
  createLottie,
  submitLottie,
  evaluateLottie,
  updateLottie,
  updateManLottie,
  gradient,
  circle,
  coneLottie,
  handLottie,
  createLottie2,
};
